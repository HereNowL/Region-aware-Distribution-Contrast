...
class RandomScaleCrop(object):
    ...
    def __call__(self, img, label, depth, sam_masks, sam_edges):
        ...
        return img_, label_, depth_ / sc, sam_masks_, sam_edges_, torch.tensor([_sc, _h, _w, _i, _j, height, width])
...
class Cityscapes_crop(Dataset):
    ...
    def __getitem__(self, index):
        ...
        semantic = torch.from_numpy(np.load(self.data_path + '/label_7/{:d}.npy'.format(index)))
        depth = torch.from_numpy(np.moveaxis(np.load(self.data_path + '/depth/{:d}.npy'.format(index)), -1, 0))
        sam_masks = cv2.imread(self.data_path + '/sam_GRAY/{:d}.png'.format(index))
        ...
        if self.augmentation and self.aug_twice == False:
            ...
            image, semantic, depth, sam_masks, sam_edges, _ = RandomScaleCrop()(image, semantic, depth, sam_masks, sam_edges)
            if self.flip:
                if torch.rand(1) < 0.5:
                    image1 = torch.flip(image, dims=[2])
                    semantic1 = torch.flip(semantic, dims=[1])
                    ...
                    image1, semantic1, depth1, sam_masks1, sam_edges1, flip = image, semantic, depth, sam_masks, sam_edges, 0
            image1, semantic1, depth1, sam_masks1, sam_edges1, trans_params = RandomScaleCrop()(image1, semantic1, depth1, sam_masks1, sam_edges1)
            return image.type(torch.FloatTensor), semantic.type(torch.FloatTensor), depth.type(torch.FloatTensor), sam_masks.type(torch.FloatTensor), sam_edges.type(torch.FloatTensor), index, image1.type(torch.FloatTensor), semantic1.type(torch.FloatTensor), depth1.type(torch.FloatTensor), sam_masks1.type(torch.FloatTensor), sam_edges1.type(torch.FloatTensor), trans_params, flip
        if self.train:
            return image.type(torch.FloatTensor), semantic.type(torch.FloatTensor), depth.type(torch.FloatTensor), index
        else:
            return image.type(torch.FloatTensor), semantic.type(torch.FloatTensor), depth.type(torch.FloatTensor)
